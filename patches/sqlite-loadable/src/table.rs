...
fn determine_update_operation<'a>(
    ...
    // If argv[1] is NULL, SQLite assigns the rowid. Otherwise, argv[1] is the explicit rowid.
    ...
    // Note: We compare values, not pointers, since argv[0] and argv[1] will have the same rowid value for same-rowid updates
    else if value_type(argv0) == value_type(argv1)
        && value_type(argv0) == ValueType::Integer
        && crate::api::value_int64(argv0) == crate::api::value_int64(argv1)
    {
    ...
    // the new rowid in argv[1] and new values in argv[2] and following parameters."
        ...
        // For now, treat rowid-changing UPDATE as a regular update
        // The caller can handle the rowid change if needed
        UpdateOperation::Update {
            _values: args
                .get(2..)
                .expect("argv[2..] should contain column values for UPDATE operations"),
